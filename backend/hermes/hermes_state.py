...
# Noah adaptation: use relative import + inline sanitize_context
# Original: from agent.memory_manager import sanitize_context
# Original: from hermes_constants import get_hermes_home
# Vendored from NousResearch/hermes-agent (MIT) — adapted for Noah's backend
...
# ─── Inline minimal sanitize_context (from agent/memory_manager.py) ──────────
_INTERNAL_CONTEXT_RE = re.compile(r"<memory-context>.*?</memory-context>", re.DOTALL)
_INTERNAL_NOTE_RE = re.compile(r"<internal-note>.*?</internal-note>", re.DOTALL)
_FENCE_TAG_RE = re.compile(r"</?(?:memory-context|internal-note)>")
...
# Noah adaptation: store state inside the backend data/ directory.
# NOAH_HERMES_DB_PATH overrides the path; defaults to backend/data/hermes_sessions.db.
