...
app.include_router(noah_hermes.router, prefix="/api/v1")

# Log active AI brain mode at startup so it's visible in server logs
_brain_mode = os.environ.get("NOAH_BRAIN_MODE", "classic").lower()
_hermes_model = os.environ.get("NOAH_HERMES_MODEL", "claude-opus-4-20250514")
if _brain_mode == "hermes":
    logging.getLogger(__name__).info(
        "Noah AI brain: HERMES mode active (model=%s) — /hermes/chat enabled",
        _hermes_model,
    )
else:
    logging.getLogger(__name__).info(
        "Noah AI brain: CLASSIC mode (set NOAH_BRAIN_MODE=hermes to enable Hermes)"
    )
