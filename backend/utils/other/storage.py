...
try:
    ...
    opuslib = None  # type: ignore[assignment]  # Opus not available on this platform
...
def encode_pcm_to_opus(pcm_data: bytes, sample_rate: int = OPUS_SAMPLE_RATE, channels: int = OPUS_CHANNELS) -> bytes:
    ...
    Raises RuntimeError if the native Opus library is not installed.
    ...
    """
    if opuslib is None:
        raise RuntimeError("Opus library not available on this platform. Install libopus via your system package manager.")
