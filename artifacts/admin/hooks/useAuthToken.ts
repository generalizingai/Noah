...
/** Prefix relative /api paths with /admin so Vite proxy routes them to Next.js at port 9000 */
...
export const authenticatedFetcher = async ([url, token]: [string, string]) => {
  const resolvedUrl = withAdminBase(url);
  ...
  if (response.status === 401 && _forceRefreshCallback) {
    ...
    if (freshToken) {
      const retryResponse = await fetchWithTimeout(resolvedUrl, {
        headers: {
...
export function useAuthFetch() {
  ...
  const fetchWithAuth = useCallback(async (url: string, init?: RequestInit) => {
    ...
    const response = await fetchWithTimeout(resolvedUrl, { ...init, headers });
    ...
    if (response.status === 401 && tokenRef.current) {
      ...
      if (freshToken) {
        ...
        return fetchWithTimeout(resolvedUrl, { ...init, headers });
      }
    }
